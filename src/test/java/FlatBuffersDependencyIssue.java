...
public class FlatBuffersDependencyIssue {
    ...
    public void test() throws Exception {
        ...
        List<Field> fields = Collections.singletonList(
                new Field("test", FieldType.nullable(new ArrowType.FloatingPoint(FloatingPointPrecision.SINGLE)), new ArrayList<>()));
        ...
        List<FieldVector> vectors = Collections.singletonList(float4Vector);

        VectorSchemaRoot root = new VectorSchemaRoot(s,vectors,10);

//        DictionaryProvider p = new DictionaryProvider.MapDictionaryProvider(new Dictionary(float4Vector, fields.get(0).getDictionary()));
